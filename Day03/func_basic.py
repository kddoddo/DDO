...
- 함수는 한 번 정의해 두면 지정된 함수 이름을 통해 언제든지 해당 코드 블록을 실행
- 함수를 정의할 때는 def라는 키워드를 사용
- 정의해 놓은 함수를 사용하는 것을 호출(call) 이라고 부름
- 파이썬에서는 함수를 호출하려면 반드시 호출문보다 상단부에 함수를 먼저 정의해야 함
'''
...
- 인수는 함수를 호출할 때 함수 실행에 필요한 값들을 전달하는 매개체 역할을 하며, 
  그렇기 때문에 매개변수(parameter)라고도 부름
- 인수의 개수는 제한이 없어 많은 값을 함수에 전달할 수도 있고 하나도 전달하지 않을 수 있음
- 파이썬의 경우에는 데이터 타입을 작성하지 않기 때문에
  이 함수를 처음 사용하는 사람도 인수 이름만 보고 무슨 값을
  전달해야 할 지 의미를 알기 쉽게 지정하는 것이 좋음
'''
...
- 반환값이란 함수를 호출한 곳으로 함수의 최종 실행 결과를 전달하는 값
- 인수는 여러 개 존재할 수 있지만, 반환값은 언제나 하나만 존재
- 모든 함수가 반환값이 있는 것은 아니다
  함수 실행 후에 딱히 반환할 값이 없다면 return을 생략 가능
'''
def add(n1, n2):
    return n1 + n2
