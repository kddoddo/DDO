...
- 함수를 호출할 때는 함수 정의시에 지정한 인수의 개수만큼 값을 전달
- 하지만 가변 인수를 사용하면 하나의 인수로 여러개의 값을 받아서 처리
- 위치 가변인수는 함수 정의부에서 인수를 선언할 때 인수 앞에 * 기호를 붙여 선언하며, 
  이런 경우에 여러 개의 데이터를 튜플로 묶어서 함수 내부로 전달
'''
...
# 가변인수와 일반 인수를 동시에 사용하실 때에는 일반 인수를 반드시 키워드 인수 방식으로 전달
